	...
	<div class="box-body">
		...
		<form role="form" method="post" action="{{ route('admin.product.update', $product->id) }}" enctype="multipart/form-data">
			...
			<div class="row">
				<div class="col-md-6 col-sm-6 col-xs-12">
					<div class="form-group">
						...
						{!! $product->category !!}
						<br>
						{!! Form::select('category', [''=> 'None', 'beat' => 'Beat', 'karaoke' => 'Karaoke'], '' ); !!}
					</div>
				</div>
				<div class="col-md-6 col-sm-6 col-xs-12">
					<div class="form-group">
			...
			<div class="form-group">
				...
				{!! Form::text('name', $product->name, ['class'=>'form-control', 'placeholder' => 'Tên bài hát'] ) !!} 
			</div>

			<div class="form-group">
				<label>{!! Form::label('slug', 'Slug') !!} (*)</label>
				{!! Form::text('slug', $product->slug, ['class'=>'form-control', 'placeholder' => 'slug will show on url'] ) !!} 
			</div>

			<div class="form-group">
				<label>Giá vnđ: (*) <small>(Ex: 120.000 vnđ, bạn sẽ điền là: 120)</small></label>
				{!! Form::text('price_vnd', $product->price_vnd, ['class'=>'form-control', 'placeholder' => 'Đơn vị 1k:1000 vnđ'] ) !!} 
				<label>Giá usd: (*) </label>
				{!! Form::text('price_usd', $product->price_usd, ['class'=>'form-control', 'placeholder' => ''] ) !!} 
			</div>

			<div class="form-group">
				<label>Hình đại diện: ( <i>width x height: {!! config('image.thumnail_with').'x'.config('image.thumnail_height') !!}</i> )</label>
				@if($product->image != "")
					<p><img src="{!! url('/images/products/'.$product->image) !!}" width="100"></p>
				@endif
				{!! Form::file('image') !!}
			</div>
	          	...
	          	<div class="nav-tabs-custom">
		            ...
		            <div class="tab-content">
			            ...
			            <div class="chart tab-pane active" id="dirrect-upload" style="position: relative; height: 150px;">
							...
							<div class="form-group">
								@if($resources != null)
									@if($resources->type === "audio")
										<label>Resouces: {{ $resources->path }}</label>
									@endif
								@else
			            ...
			            <div class="chart tab-pane" id="other-upload" style="position: relative; height: 150px;">
							...
							<div class="form-group">
								@if($resources != null)
									<label>Link URL: @if($resources->type === "url") {{ $resources->path }} @endif</label>
								@else
									<label>Link URL: </label>
								@endif
								{!! Form::text('resource_url', '', ['class'=>'form-control', 'placeholder' => 'add new url of resources'] ) !!} 
							</div>
			            </div>
		            </div>
	          	</div>
